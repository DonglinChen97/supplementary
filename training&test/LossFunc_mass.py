...
class MyLoss(nn.Module):
    ...
    def forward(self, pred, trut):
        ...
        x = 2
        y = 3
        ixm = torch.arange(1, dim[x]-1).to(device) # index x-row middle
        ixl = torch.arange(0, dim[x]-2).to(device) # left
        ixr = torch.arange(2, dim[x]).to(device)   # right
        iym = torch.arange(1, dim[y]-1).to(device) # index y-row middle
        iyb = torch.arange(0, dim[y]-2).to(device) # bottom
        iyt = torch.arange(2, dim[y]).to(device)   # top
        ...
        # ip = torch.LongTensor([p]).to(device)
        iu = torch.LongTensor([u]).to(device)
        iv = torch.LongTensor([v]).to(device)
        ...
        lossMass0 = torch.abs((dudx + dvdy) - (du_dx + dv_dy)) /  torch.sum(torch.abs(dudx+dvdy))
        lossMass = torch.sum(lossMass0)
        ...
        lossMom0 = lossMom0 / torch.sum(torch.abs(du2dx  +duvdy  -(dudx2 +dudy2) *iRe) + torch.abs(dvudx  +dv2dy   -(dvdx2 +dvdy2) *iRe))
        lossMom = torch.sum(lossMom0)
        ...
        #loss = lossMass*0.2 + lossMom*0.2 + lossGdl*0.2 + lossL1*0.4
        #loss = lossMass*0.8 + lossMom*0.2 + lossGdl*0.2 + lossL1*0.4
        loss = lossMom
        return loss
